"""
This module contains the main application code for the recipe app.
It imports necessary packages and modules, loads environment variables,
...
def search_by_ingredients():
    ...
    if not all(
        idx.isnumeric() and 0 < int(idx) <= len(all_ingredients)
...
def edit_recipe():
    ...
    if attribute_to_edit == "1":
        ...
        if len(new_name) > 50 or not all(
            char.isalnum() or char.isspace() for char in new_name
        ):
            print("Invalid recipe name!")
            return
